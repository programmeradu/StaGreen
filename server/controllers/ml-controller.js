...
export const getWastePrediction = async (request, response) => {
  ...
  console.log(
    `Spawning waste prediction script: python ${scriptPath} "${area}" ${daysInt} "${modelsDir}"`
  )
  ...
  pythonProcess.on('close', (code) => {
    ...
    if (stderrData || code !== 0) {
      console.error(
        `Error during waste prediction for area "${area}": ${stderrData || `Exited with code ${code}`}`
      )
      ...
      try {
        ...
        if (errorJson.error) {
          return sendJsonResponse(response, 500, {
            error: `Prediction script error: ${errorJson.error}`
          })
        }
      } catch (e) {
      ...
      return sendJsonResponse(response, 500, {
        error: 'Error during prediction processing.'
      })
    }
    ...
    try {
      ...
      if (parsedOutput.error) {
        // Handle cases where script itself prints an error JSON to stdout
        console.error(
          `Prediction script returned an error in stdout: ${parsedOutput.error}`
        )
        return sendJsonResponse(response, 500, {
          error: `Prediction error: ${parsedOutput.error}`
        })
      }
      return sendJsonResponse(response, 200, parsedOutput)
    } catch (error) {
      console.error(
        `Error parsing waste prediction output for area "${area}": ${error.message}\nRaw output: ${stdoutData}`
      )
      return sendJsonResponse(response, 500, {
  ...
  pythonProcess.on('error', (error) => {
    console.error(
      `Failed to start waste prediction script for area "${area}": ${error.message}`
    )
    return sendJsonResponse(response, 500, {
      error: 'Failed to start prediction script.'
    })
  })
}
...
export const getOptimizedRoutes = async (request, response) => {
  ...
  try {
    ...
    const depot_location = [5.6037, -0.187] // Accra, Ghana (Example, make configurable)
    ...
    pythonProcess.on('close', (code) => {
      ...
      if (stderrData || code !== 0) {
        console.error(
          `Error during route optimization for date "${date}": ${stderrData || `Exited with code ${code}`}`
        )
        try {
          const errorJson = JSON.parse(stderrData) // Python script outputs JSON errors to stderr
          if (errorJson.error) {
            return sendJsonResponse(response, 500, {
              error: `Route optimization script error: ${errorJson.error}`
            })
          }
        } catch (e) {
          // Not a JSON error from script, or malformed
        ...
        return sendJsonResponse(response, 500, {
          error: 'Error during route optimization processing.'
        })
      }
      ...
      try {
        ...
        if (parsedOutput.error) {
          // Handle cases where script itself prints an error JSON to stdout
          console.error(
            `Route optimization script returned an error in stdout: ${parsedOutput.error}`
          )
          return sendJsonResponse(response, 500, {
            error: `Optimization error: ${parsedOutput.error}`
          })
        }
        return sendJsonResponse(response, 200, parsedOutput)
      } catch (error) {
        console.error(
          `Error parsing route optimization output for date "${date}": ${error.message}\nRaw output: ${stdoutData}`
        )
        return sendJsonResponse(response, 500, {
          error: 'Failed to parse route optimization output.'
        })
      }
    })
    ...
    pythonProcess.on('error', (error) => {
      console.error(
        `Failed to start route optimization script for date "${date}": ${error.message}`
      )
      return sendJsonResponse(response, 500, {
        error: 'Failed to start route optimization script.'
      })
    })
  } catch (dbError) {
    console.error(
      `Database error while fetching pickup requests for date "${date}": ${dbError.message}`
    )
    return sendJsonResponse(response, 500, {
