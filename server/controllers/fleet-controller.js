  ...
  return new Promise((resolve, reject) => {
    ...
    pythonProcess.on("close", (code) => {
      if (stderrData && code !== 0) {
        // Prioritize stderr for error reporting from script
        ...
        try {
          ...
          return reject({
            error: `Python script ${scriptPath} failed with code ${code}`,
      ...
      if (code !== 0) {
        console.error(
          `Python script ${scriptPath} exited with code ${code}, STDERR: ${stderrData}`,
        );
        return reject({
          error: `Python script ${scriptPath} failed with code ${code}`,
          details: stderrData.trim(),
      ...
      try {
        ...
        console.error(
          `Failed to parse JSON output from ${scriptPath}: ${stdoutData}`,
        );
        reject({
          error: `Failed to parse JSON output from ${scriptPath}`,
          details: e.message,
    ...
    pythonProcess.on("error", (err) => {
      ...
      reject({
        error: `Failed to start Python script ${scriptPath}`,
        details: err.message,
      });
    });
...
export const generateDynamicRoutes = async (request, response) => {
  ...
  const simulatorScriptPath = path.resolve(
    process.cwd(),
    "../ml/simulators/smart_bin_simulator.py",
  );
  const routingScriptPath = path.resolve(
    process.cwd(),
    "../ml/core_routing_ortools.py",
  );
  ...
  try {
    ...
    if (!simulatedPickups || simulatedPickups.length === 0) {
      return response
        .status(200)
        .json({ routes: [], status: "success_no_pickups_simulated" });
    }

    // 2. Prepare input for OR-Tools routing script
    const depotLocation = [5.6037, -0.187]; // Example for Accra - make configurable later
    const routingInput = {
    ...
    console.log(
      `Calling core_routing_ortools.py with ${simulatedPickups.length} pickups.`,
    );
    const optimizedRouteData = await runPythonScript(
      routingScriptPath,
      [],
      routingInput,
    );
    ...
    if (
      optimizedRouteData &&
      optimizedRouteData.status &&
      optimizedRouteData.status.startsWith("error_")
    ) {
    ...
    const errorPayload = error.error
      ? error
      : {
          error: "Failed to generate dynamic routes",
          details: error.message || error,
        };
    // Determine appropriate status code based on error origin if possible
    const statusCode =
      error.error &&
      (error.status === "error_demand_exceeds_capacity" ||
        error.details?.includes("ValueError"))
        ? 400
        : 500;
