...
const ErrorMessage = ({ error, title, sx }) => {
  ...
  if (typeof error === 'string' && error.trim() !== '') {
    ...
    // For backend errors like { error: "message" }
  ...
  return (
    <Alert severity="error" sx={{ my: 2, ...sx }}>
      {' '}
      {/* Default margin, can be overridden by sx prop */}
...
ErrorMessage.propTypes = {
  error: PropTypes.oneOfType([
    PropTypes.string,
    PropTypes.shape({
      message: PropTypes.string, // For error objects with a message property
      error: PropTypes.string, // For backend error objects like { error: "message" }
    }),
    PropTypes.object, // Catch-all for other error object shapes
  ]),
